/**
 * @name NoSQL injection worse (boosted)
 * @description Boosted variant of NoSQL injection worse
 ...
 */
DataFlow::Node getASubexpressionWithinQuery(DataFlow::SourceNode query) {
...
class NosqlInjectionWorseATMConfig extends ATMConfig {
  NosqlInjectionWorseATMConfig() { this = "NosqlInjectionWorseATMConfig" }
...
where
  ...
  source
      .hasLocationInfo(filePathSource, startLineSource, startColumnSource, endLineSource,
        endColumnSource) and
  scoreForFlow(source, sink) = score
