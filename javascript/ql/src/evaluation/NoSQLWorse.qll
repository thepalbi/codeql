/**
 * Same as `NoSQL.qll` query from `github/codeql` but with fewer sinks (reverts `2a87d53` in `ql` repo)
 */
...
module NoSQLWorse {
  ...
  abstract class QueryCallWorse extends DatabaseAccess { }
}
...
private module MongoDB {
   ...
   */
  class Query extends NoSQLWorse::Query {
    Query() { this = any(QueryCall qc).getAQueryArgument().asExpr() }
  }

  class QueryCallWorse extends NoSQLWorse::QueryCallWorse {
    QueryCallWorse() { this = any(QueryCall qc)}

    override DataFlow::Node getAQueryArgument() { result = any(QueryCall qc).getAQueryArgument() }
  }
