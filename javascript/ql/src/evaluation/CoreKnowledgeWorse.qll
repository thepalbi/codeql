/**
 * Provides predicates that expose the knowledge of models in the core CodeQL JavaScript libraries.
 *
 * This file is identical to `CoreKnowledge.qll` except for reverting the modeling of NoSQL
 * libraries (effectively reverting 2a87d53 in ql/ repo). We use it in `NosqlInjectionWorseATM.ql`
 * so that the knowledge of models we have matches the worse modeling of the NoSQL libraries in
 * `NosqlInjectionWorse.ql` and `NosqlInjectionWorseATM.ql`.
 */
...
private import NosqlInjectionWorseCustomizations
...
predicate isKnownLibrarySink(DataFlow::Node n) {
  ...
  n instanceof NosqlInjectionWorse::Sink or
