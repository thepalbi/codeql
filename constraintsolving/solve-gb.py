...
try:
    ...
    m.write("models/{0}/gurobi_model.lp".format(projectdir))
    m.write("models/{0}/gurobi_model.mps".format(projectdir))
    # Optimize model
    m.optimize()
    zero=0
    non_zero=0
    ones=0
    with open("models/{0}/results_gb.txt".format(projectdir), "w") as resultfile:
        for v in m.getVars():
